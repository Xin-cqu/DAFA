...
'''
Load data from Matlab: PC of the slice-windows of fMRI(220young-192old), window's length is 50 time point.
...
class AutoEncoder(nn.Module):
    ...
    #encoded = nn.functional.dropout(encoded,p=0.8,training=self.training)
        decoded = self.decoder(encoded)
    #decoded = nn.functional.dropout(decoded,p=0.5,training=self.training)
...
for epoch in range(EPOCH):
    ...
    #print('Batch size: ', batchsize, '| train loss: %.4f' % loss.item())
# print(loss.data[0])            # print(encoded)
    #print('Save the encoder for feature selection.\n')
torch.save(autoencoder.encoder,'aging_160.pkl')
